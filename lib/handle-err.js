...
`,
  connectionFailed: `
Oops! It looks like we couldn't connect to SSB. This module doesn't run a
server, so you need to start one in the background. Currently the simplest way
to do this is to open an SSB app or run \`ssb-server start\`.

If you already have a server running, use environment variables to configure
the host and port of the destination server. For example:

    SSB_HOST=example.com SSB_PORT=1234 ssb whoami
`,
};
